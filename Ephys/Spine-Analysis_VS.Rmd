...
d = aggregate(data, by=list(data$Mouse, data$Group, data$Region, data$Cell), FUN=mean, na.rm=TRUE)
d[,5:8]=NULL
colnames(d)=c("Mouse","Group","Region","Cell","density")
d  %>% filter(Region=="vs") %>% group_by(Group) %>% summarise(mouse = n_distinct(Mouse), cells=n())
