---
title: "ATAC_V3_MIA vs CON at P4"
author: "LH"
date: "3/29/2021"
...
MvCinP4 <- results(DDS, list( c("Condition_MIA_vs_CON","ConditionMIA.TreatmentP4")))
```

### Permutation Test for MIA vs CON at P4
```{r}
samples <- read.csv("ATAC_sampleSheet_34.csv", header = TRUE, stringsAsFactors = TRUE)
table(MvCinP4$pvalue<0.05, dnn="p-value sig")
table(MvCinP4$padj<0.05, dnn="FDR sig")
```
...
```
```{r permutation}
p4 <- samples$Treatment == "P4"
edata <- counts[,p4]
group <- samples$Condition[p4]
...
plot(t3, col="red", xlim=c(-2,5), ylim=c(0,10000), lty="blank")
plot(t1, col="black",  add=T, lty="blank")
```

### calculates the percentage of null t-values that are more extreme than the test t-value and that becomes the new p-value. 
```{r}
...
# 464 is more than the previous 347 from DESeq which makes sense because its taking each comparison from its own null distribution. 
```
...
```{r}
sig <- bh<0.05
df <- data.frame((MvCinP4))
CON <- edata[,group=="CON"]
MIA <- edata[,group=="MIA"]
df$count.con.p4 <- apply(CON,1,mean)
df$count.mia.p4 <- apply(MIA,1,mean)
df$count.fc <- log2(df$count.mia.p4/df$count.con.p4)
