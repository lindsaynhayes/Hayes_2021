...
gwas.all <- GWAS_cat[grep("bipolar", GWAS_cat$disease),]
...
fit <- chisq.test(mat)
fit
print(paste0(round(pct.DEG, 2), '% of DEG are associated with BPD'))
print(paste0(round(pct.notDEG, 2), '% of the expressed but not DEG are associated with BPD'))
